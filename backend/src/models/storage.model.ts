...
interface StorageStatistics {
  getStorageMetrics(userId: Types.ObjectId): Promise<StorageMedtrics>;
  validateUpload(
    userId: Types.ObjectId,
    fileSize: number,
...
interface StorageModelType extends Model<StorageDocument>, StorageStatistics {}
...
const StorageSchema = new Schema<StorageDocument, StorageModelType>(
  {
    ...
    storageQuota: {
      ...
      min: [0, "Storage quota can't be negative"],
    },
  },
  {
    timestamps: true,
...
StorageSchema.statics = {
  ...
  async validateUpload(userId: Types.ObjectId, totalFileSize: number) {
    if (totalFileSize < 0)
      throw new BadRequestException("File size can't be negative");
